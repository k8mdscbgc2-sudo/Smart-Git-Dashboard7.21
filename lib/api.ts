...
// 浏览器永远走相对路径 /api/v1/*，由 next.config.js rewrite 反代到后端。
// 这样不会触发浏览器 CORS。Next.js 读取的是 BACKEND_API_BASE_URL（服务端私有 env）。
// 本地开发：如果 next.config.js 里启用了重写（只要 BACKEND_API_BASE_URL 有值）则无需 CORS。
